// get csv file
d3.csv('practice9_scatterBrush/movies.csv', type).then(
    res => {
        ready(res)
        // console.log(res)
    }
...
function setupCanvas(scatterData) {
    ...
    function hightlightSelected(data) {
        ...
        d3.selectAll('.scatter').filter(d => selectedIDs.includes(d.id)).style('fill', 'green')
        d3.selectAll('.scatter').filter(d => !selectedIDs.includes(d.id)).style('fill', 'dodgerblue')
